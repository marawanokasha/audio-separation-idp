...
dataDirectory = 'sisec_code/saved_scatts/und/';
...
%%
save('sisec_code/saved_dicts/und/renorm_params.mat', 'stds1', 'stds2')
...
scatts = [];
load('sisec_code/saved_scatts/und/scatt_all_non_vectorized_non_renorm_X1_X2_male_speaker_m1.mat')
reshapedX1 = reshape(X1, [size(X1,1), size(X1,2) * size(X1,3)]);
reshapedX2 = reshape(X2, [size(X2,1), size(X2,2) * size(X2,3)]);
scatt.X1 = reshapedX1;
scatt.X2 = reshapedX2;
...
scatts = [scatts scatt];
load('sisec_code/saved_scatts/und/scatt_all_non_vectorized_non_renorm_X1_X2_male_speaker_m2.mat')
reshapedX1 = reshape(X1, [size(X1,1), size(X1,2) * size(X1,3)]);
reshapedX2 = reshape(X2, [size(X2,1), size(X2,2) * size(X2,3)]);
scatt.X1 = reshapedX1;
scatt.X2 = reshapedX2;
...
scatts = [scatts scatt];
load('sisec_code/saved_scatts/und/scatt_all_non_vectorized_non_renorm_X1_X2_male_speaker_m3.mat')
reshapedX1 = reshape(X1, [size(X1,1), size(X1,2) * size(X1,3)]);
reshapedX2 = reshape(X2, [size(X2,1), size(X2,2) * size(X2,3)]);
scatt.X1 = reshapedX1;
scatt.X2 = reshapedX2;
...
for i=1:length(scatts)
    ...
    save(strcat('sisec_code/saved_dicts/und/dict_all_non_vectorized_non_renorm_', scatts(i).identifier, '.mat'),'Dnmf1','Dnmf2')
