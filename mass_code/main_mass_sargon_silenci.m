...
dataDirectory = '../data/MASS/sargon-silenci (metal)/sargon-silenci_1-19';
saveDirectory = strcat('final/mass/sargon_silenci/','T_',int2str(T),'_D1_',int2str(dictionaryAtomsX1),'_D2_',int2str(dictionaryAtomsX2),'_fs_',int2str(fs),'/');
...
for i=1:size(sourceNameList,1)
    ...
    file_content = resample(file_content,trainingFs,fs);
...
[testSourcesL,testSourcesR,testSourceNameList,testMixL,testMixR,tfs,nbits] = loadSources(testDirectory,0);

testSources = [];

for i=1:size(testSourceNameList,1)
    file_content = ((testSourcesL(:,i) + testSourcesR(:,i))/2);
    file_content = resample(file_content,tfs,fs);
    file_content = pad_mirror(file_content,Npad);
...
[speech_list, lvl_1_speach_list] = demix_scatt2top_multi(testMix, Dnmf1_list, Dnmf2_list, stds1, stds2, eps, filts, scparam, param1, param2, Npad);
